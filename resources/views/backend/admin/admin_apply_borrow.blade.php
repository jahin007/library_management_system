    ...
    <div class="row-fluid sortable">
        <div class="box span12">
            <div class="box-header" data-original-title>
                <h2><i class="halflings-icon user"></i><span class="break"></span>Applied Books</h2>
                <div class="box-icon">
                ...
                <table class="table table-striped table-bordered bootstrap-datatable datatable">
                    ...
                    @foreach($borrows as $borrow)
                        ...
                        <tr>
                            ...
                            <td class="center" style="color: black;"><b>{{$borrow->status}}</b></td>
                            <td>
                                @foreach($borrow->book->images as $image)
                                    <img src="{{asset('/images/books/'.$image->image)}}" alt="" style="height: 50px; width: 50px;">
                                @endforeach
                            </td>
                            <td>@if($borrow->status == 'Applied')
                                    <a class="btn btn-success" href="{{route('approve_book',$borrow->id)}}">Approve</a>
                                    <a class="btn btn-danger" href="{{route('reject_book',$borrow->id)}}">Reject</a>
                                @endif
                            </td>
                        </tr>
                        </tbody>
